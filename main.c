...
void display_board(int board[ROW_MAX][COL_MAX], struct last_moves_archive *archive) {
    ...
    for (int row_count = 0; row_count < ROW_MAX; row_count++) {
        ...
        for (int col_count = 0; col_count < COL_MAX; col_count++) {
            ...
            int found = 0;
            int i;
            int publisher = board[row_count][col_count];

            for (i = 0; i < archive->size; i++) {
                if(archive->data[i].last_row == row_count && archive->data[i].last_col == col_count) {
                    found = 1;
                    break;
                }
            }
...
// ! PAP
void play(int* inputs, int board[ROW_MAX][COL_MAX], struct last_moves_archive *archive, int game_move, int solution[ROW_MAX][COL_MAX]) {
        ...
        switch (mode) {
            case 1: // undo
                if(game_move != 0) {
                    ...
                    display_board(board, archive);
                    play(input_interaction(), board, archive, game_move-1, solution);
                }
                break;
    ...
    if(display_solution) {
        display_board(solution, archive);
    }
    ...
    display_board(board, archive);
    play(input_interaction(), board, archive, game_move, solution);
}

void sudoku() {
    int board[ROW_MAX][COL_MAX] = {{0}};
    int solution[ROW_MAX][COL_MAX] = {{0}};
    ...
    if (build_sudoku(board, 0, 0, available_numbers)) { // ! PAP
        ...
        if (blackening_sudoku(board, ((ROW_MAX * COL_MAX) - NUM_OF_FIELDS_MIN))) {
            display_board(board, archive);
            play(input_interaction(), board, archive, 0, solution);
        }
    } else {
        // error handling
    }
